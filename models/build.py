...
class HeadAndLoss(nn.Module):
    ...
    def forward(self, deep_features, labels, rank=-1): 
        ...
        #     'spwmin': sp.min().item(),
        #     'spwmax': sp.max().item(),
        #     'snwmin': sn_prime.min().item() if 'unpg' in self.aux_name else sn.min().item(),
        #     'snwmax': sn_prime.max().item() if 'unpg' in self.aux_name else sn.max().item(),
        # }
        metas = {
        'snmin': float(sn.min().item()) if sn.numel() > 0 else 0.0,  # Handle empty tensor
        'snmax': float(sn.max().item()) if sn.numel() > 0 else 0.0,  # Handle empty tensor
        'spwmin': float(sp.min().item()) if sp.numel() > 0 else 0.0,  # Handle empty tensor
        'spwmax': float(sp.max().item()) if sp.numel() > 0 else 0.0,  # Handle empty tensor
        'snwmin': float(sn_prime.min().item()) if 'unpg' in self.aux_name and sn_prime.numel() > 0 else float(sn.min().item()) if sn.numel() > 0 else 0.0,  # Handle empty tensor
        'snwmax': float(sn_prime.max().item()) if 'unpg' in self.aux_name and sn_prime.numel() > 0 else float(sn.max().item()) if sn.numel() > 0 else 0.0,  # Handle empty tensor
        }
